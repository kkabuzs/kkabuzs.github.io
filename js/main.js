(function () {
  ...
  function setupCategoryWorkbench() {
    var root = document.querySelector('.category-workbench');
    if (!root) return;
    var list = root.querySelector('.wb-list');
    if (!list) return;
    var items = Array.prototype.slice.call(list.querySelectorAll('.wb-item'));
    if (!items.length) return;

    var searchInput = root.querySelector('.wb-search-input');
    var tagButtons = Array.prototype.slice.call(root.querySelectorAll('.wb-tag'));
    var sortButtons = Array.prototype.slice.call(root.querySelectorAll('.wb-sort-btn'));
    var countEl = root.querySelector('.wb-result-count');
    var emptyEl = root.querySelector('.wb-empty');
    var total = items.length;
    var state = { query: '', tag: '', sort: 'new' };
    ...
    function applyFilter() {
      ...
      items.forEach(function (item) {
        ...
        var matchTag = !state.tag || tags.indexOf('|' + state.tag.toLowerCase() + '|') !== -1;
        var show = matchQuery && matchTag;
        item.hidden = !show;
        if (show) visible += 1;
      });
      if (countEl) {
        countEl.textContent = (state.query || state.tag)
          ? '筛选到 ' + visible + ' 篇'
          : '共 ' + total + ' 篇';
      }
  ...
  function getSearchContext() {
    var path = normalizePath(window.location.pathname).replace(/page\/\d+\/$/, '');
    if (path === '/categories/') {
      return {
        type: 'category-index',
        label: '搜索分类',
